...
public class SingleplayerGameView extends GameView {
    ...
    private AIController AIPlayerOne;
    private AIController AIPlayerTwo;
    private AIController AIActualPlayer;
    private InputEvent event1 = new InputEvent();
    private InputEvent event2 = new InputEvent();
    ...
    private Table playerActualCards;

    private HashMap<String, Actor> hashButtons = new HashMap<>();
     ...
     * @param next_screen
     */
    SingleplayerGameView(ScreenAdapter next_screen) {

        super(next_screen);

        AIPlayerOne = new AIController();
        AIPlayerTwo = new AIController();
        myPlayer = getInstance().getJoinedPlayers().get(0);
    ...
    private void loadPlayerCards() {
        ...
        ArrayList<Card> playerCards = getInstance().getPlayerByCharacter(getInstance().getJoinedPlayers().get(0)).getCards();
        ArrayList<ImageButton> playerButtonsCards = new ArrayList<>();

        playerActualCards = new Table();
        playerActualCards.setFillParent(true);
        playerActualCards.bottom();
        playerActualCards.padRight(board.getWidth());
        playerActualCards.padBottom(actualText.getPrefHeight());
        ...
        for (Card c : playerCards) {
            ...
            switch (c.toString()) {
                ...
                case "Hall":
                    ...
                    ImageButton hallCardButton = new ImageButton(new SpriteDrawable(atlas.createSprite("Location_Hall")));
                ...
                case "Study":
                    ...
                    ImageButton studyCardButton = new ImageButton(new SpriteDrawable(atlas.createSprite("Location_Study")));
        ...
        if (!playerButtonsCards.isEmpty()) {
            playerActualCards.add(playerButtonsCards.get(0)).expandX().spaceBottom(25);
            playerActualCards.add(playerButtonsCards.get(1)).expandX().spaceBottom(25);
            playerActualCards.add(playerButtonsCards.get(2)).expandX().spaceBottom(25);
            playerActualCards.row();
            playerActualCards.add(playerButtonsCards.get(3)).expandX();
            playerActualCards.add(playerButtonsCards.get(4)).expandX();
            playerActualCards.add(playerButtonsCards.get(5)).expandX();

            playerActualCards.setVisible(false);

            stage.addActor(playerActualCards);
        }
    }
    ...
    private void setupBackCardsListener(ImageButton cardButton) {
        ...
        cardButton.addListener(new ClickListener() {
            ...
            public void clicked(InputEvent event, float x, float y) {
                ...
                playerActualCards.setVisible(true);
            }
        });
    }
    ...
    private void initMultiplexer() {
        ...
        multiplexer.addProcessor(new GestureDetector(new GestureDetector.GestureAdapter() {
            ...
            public boolean fling(float velocityX, float velocityY, int button) {
                ...
                if (getInstance().getCurrentPlayer() == getInstance().getJoinedPlayers().get(0)) {
                    moved = movePlayer(velocityX, velocityY);
                }
    ...
    /**
     * adds the exit buttons to the hashtable
     */
    private void addExitButtonsToHash() {

        hashButtons.put("Ballroom0", ballRoomOne);
        hashButtons.put("Ballroom1", ballRoomTwo);
        hashButtons.put("Ballroom2", ballRoomThree);
        hashButtons.put("Ballroom3", ballRoomFour);

        hashButtons.put("Billiard Room0", billiardExitOne);
        hashButtons.put("Billiard Room1", billiardExitTwo);

        hashButtons.put("Library0", libraryOne);
        hashButtons.put("Library1", libraryTwo);

        hashButtons.put("Hall0", hallOne);
        hashButtons.put("Hall1", hallTwo);
        hashButtons.put("Hall2", hallThree);

        hashButtons.put("Dinning Room0", dinningExitOne);
        hashButtons.put("Dinning Room1", dinningExitTwo);
    }
    ...
    private void verifyPosition() {
        ...
        if (getInstance().getBoard().lookForEntrance(player) != null && atHallway && !rollDices) {
            ...
            if (getInstance().getCurrentPlayer() != getInstance().getJoinedPlayers().get(0)) {
                ...
                for (Room r : AIActualPlayer.getVisitedRooms()) {

                    if (getInstance().getBoard().lookForEntrance(player).toRoom() == r) {
    ...
    /**
     * makes the tables untouchable
     */
    private void disableTouchable() {

        dicesTable.setTouchable(Touchable.disabled);
        moveTable.setTouchable(Touchable.disabled);
        entranceTable.setTouchable(Touchable.disabled);
        enterRoomTable.setTouchable(Touchable.disabled);
        roomTable.setTouchable(Touchable.disabled);
        roomSecretTable.setTouchable(Touchable.disabled);
    }

    /**
     * makes the tables touchable
     */
    private void enableTouchable() {

        dicesTable.setTouchable(Touchable.enabled);
        moveTable.setTouchable(Touchable.enabled);
        entranceTable.setTouchable(Touchable.enabled);
        enterRoomTable.setTouchable(Touchable.enabled);
        roomTable.setTouchable(Touchable.enabled);
        roomSecretTable.setTouchable(Touchable.enabled);
    }
    ...
    public void render(float delta) {
        ...
        character = getInstance().getCurrentPlayer();
        player = getInstance().getPlayerByCharacter(character);
        ...
        if (getInstance().getCurrentPlayer() != myPlayer) {

            AITurn(delta);

        } else enableTouchable();

        batch.begin();
        board.draw(batch);

        for (Sprite s : characterSprites) s.draw(batch);
        ...
        if (movePlayer) moveTable.setVisible(true);
    ...
    private void AITurn(float delta) {

        disableTouchable();

        if(character == getInstance().getJoinedPlayers().get(getInstance().getJoinedPlayers().size()-2)) AIActualPlayer = AIPlayerOne;

        if(character == getInstance().getJoinedPlayers().get(getInstance().getJoinedPlayers().size()-1)) AIActualPlayer = AIPlayerTwo;

        if (rollDices) AIRollDices(delta);

            if (movePlayer) AIMove(delta);
            ...
            if (atRoom) AIMakeSuggestion(delta);

            if(exitRoom) AIExitRoom(delta);


}

    private void AIExitRoom(float delta) {

        if (counter >= 2.5) {
            counter = 0.0;
            AIActualPlayer.clearPath();
            AIActualPlayer.getVisitedRooms().add(getInstance().getBoard().lookForExit(player).get(0).toRoom());
            if (getInstance().getBoard().lookForSecPas(player) != null) {

                secretExitRoomButton.fire(event1);
                secretExitRoomButton.fire(event2);
                atRoom = false;


            } else {

                roomExitRoomButton.fire(event1);
                roomExitRoomButton.fire(event2);
                atRoom = false;

                int i = rand.nextInt(getInstance().getBoard().lookForExit(player).size());

                hashButtons.get(getInstance().getBoard().lookForExit(player).get(i).toRoom().toString() + i).fire(event1);
                hashButtons.get(getInstance().getBoard().lookForExit(player).get(i).toRoom().toString() + i).fire(event2);
     ...
     * @param delta
     */
    private void AIMakeSuggestion(float delta) {

        if (counter >= 3.5) {
            counter = 0.0;
            enterRoomTable.setVisible(false);
            actualText.setText(getInstance().getCurrentPlayer().toString() + "'s making a suggestion");
            AIActualPlayer.setSuggestion();
            getInstance().setAccusation();
            if (getInstance().getKnownCards().size() == 18) {

                makeAccusation();
                return;
            }
            getInstance().refuteSuggestion();
            endTurn();
            atRoom = false;
            atHallway = true;
        } else {
    ...
    /**
     * processes ai movement
     * @param delta
     */
    private void AIMove(float delta) {

        ArrayList<Integer> path = AIActualPlayer.getPath();
        ...
        if (counter >= 1.0) {
            ...
            if (!path.isEmpty()) {
                ...
                AIActualPlayer.getPath().remove(0);
    ...
    /**
     * rools dices for a ai player
     * @param delta
     */
    private void AIRollDices(float delta) {

        if (counter >= 1.5) {
            counter = 0.0;

            rollDiceButton.fire(event1);
            rollDiceButton.fire(event2);
